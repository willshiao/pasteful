...
function newPaste (req, res, next) {
  if (req.is('text/plain')) {
    ...
    if (Buffer.byteLength(req.body, 'utf8') > config.get('paste.maxSize')) { return next(new ArgumentError('Paste too large.')) }
    Paste.newPaste({
      content: req.body,
      ip: req.ip
    }).save()
    ...
    if (!req.body.content) { return next(new ArgumentError('No content field found')) }
    if (Buffer.byteLength(req.body.content, 'utf8') > config.get('paste.maxSize')) { return next(new ArgumentError('Paste too large.')) }
...
function getPaste (req, res, next) {
  ...
  if (!shortid.isValid(id)) { return next(new NotFoundError('Paste not found.')) }
  ...
  Paste.where('slug', id)
    ...
    .exec()
    .then(paste => {
      if (paste === null) { return next(new NotFoundError('Paste not found.')) }
      res.type('text')
